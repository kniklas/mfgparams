...
| `ci-ok` | **Aggregates every row above except the CodeQL contexts.** Issue #75 P2.4: `main`'s ruleset named all 11 workflow jobs individually, so each rename or matrix change had to be mirrored into the ruleset and into 12 committed files — #71's `test` → `test (3.9)`…`test (3.12)` rename took three commits and still missed `.github/pull_request_template.md`. `ci-ok` `needs:` the nine gating jobs (the eight above plus `changes`, the path-selection filter producer added by specs/016-ci-path-based-selection) and asserts each result explicitly (`if: always()` does not fail a job implicitly, so an unasserted aggregate reports green while `lint` is red). `performance`, `quality-summary`, `deploy-docs` and `sync-agent-integrations` are deliberately excluded, enforced by `tests/static/test_ci_ok_aggregate_check.py` | push, pull_request | Any of the nine gating jobs failing or being cancelled. Being **skipped** no longer blocks as of specs/016-ci-path-based-selection: `lint`/`complexity`/`typecheck`/`security`/`test`/`build`/`docs` are now conditional on which paths a pull request touched (see that spec's contracts/path-selection-contract.md), and an intentional skip must not block a merge that has nothing for the skipped job to check. `changes` itself has no `needs:` of its own, so on a non-scheduled run it can only report `success` or `failure` — never `skipped` — meaning this relaxation cannot itself be used to bypass a gate that should have run. |
