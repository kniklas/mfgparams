...
- [X] T035 [P] Populate `material_number`/`short_notation` in `src/mfgparams/data/materials.toml` for as many of the 6 bundled metal materials as have a known EN/DIN designation (plan.md Project Structure; contracts/materials-config-schema-delta.md) — a data-entry task, not an interaction-behavior change
...
- [X] T049 Add a unit test in `tests/unit/shared/test_registry_material_notations.py` asserting the real bundled registry (`get_material(name)`, no config override) reports `material_number`/`short_notation` matching the values T035 populated for Mild Steel/Stainless Steel/Brass, and `None` for both on Aluminum/Cast Iron/Titanium — mirroring `test_registry_material_types.py`'s `TestBundledCategorization` precedent, which tests real bundled values directly rather than only synthetic `tmp_path` configs per T035 (missing)
