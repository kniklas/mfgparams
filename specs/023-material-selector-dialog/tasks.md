...
- [X] T035 [P] Evaluate `material_number`/`short_notation` population for each of the 6 bundled metal materials in `src/mfgparams/data/materials.toml` against a known EN/DIN designation (plan.md Project Structure; contracts/materials-config-schema-delta.md) — a data-entry task, not an interaction-behavior change. **Outcome (PR #106 review, rounds 1-2)**: none qualify, per the review's reasoning that each bundled name is a generic material family, not a single grade unambiguous enough across standards traditions to stamp a specific EN/DIN number onto — both fields left `None` on every bundled entry at that point. **Superseded by T052/T053**: real manual testing after the review loop found this made the picker have nothing real to search out of the box at all — a genuine usability regression the review loop's iterative caution had over-corrected into. The final state (T053) populates every bundled metal with its single most conventional EN/DIN designation instead, treated as an explicitly *representative* default rather than a claim of exact, unambiguous equivalence.
...
- [X] T049 Add a unit test in `tests/unit/shared/test_registry_material_notations.py` asserting the real bundled registry (`get_material(name)`, no config override) reports `material_number`/`short_notation` matching each bundled metal's documented value — mirroring `test_registry_material_types.py`'s `TestBundledCategorization` precedent, which tests real bundled values directly rather than only synthetic `tmp_path` configs per T035 (missing). **Superseded by T053**: this test's exact expected values (and whether they're `None` or populated) tracks whichever bundled-data decision is current — see T035/T052/T053's history.
...
- [X] T052 Add a 7th bundled metal, `S235JR Structural Steel`, to `src/mfgparams/data/materials.toml` with `material_number = "1.0038"`/`short_notation = "S235JR"` populated — its own name states the specific grade, so (unlike the generic-family entries T035 left blank) there is no ambiguity to misrepresent. Reuses "Mild Steel"'s existing machining constants (25.0/0.20/1900.0), since S235JR is itself a mild/structural steel in that same class. Updated: `materials.toml`'s top-of-file comment; `_EXPECTED_BUNDLED_MATERIALS`/expected-name-order assertions in `test_registry.py`/`test_registry_config.py`; `_METALS` in `test_registry_material_types.py`; `TestBundledNotations` in `test_registry_material_notations.py` (split into generic-blank vs. grade-specific-populated cases); `quickstart.md`'s fixture (no longer needs to redeclare this entry itself) and its dependent candidate counts; `README.md`'s metal-materials list. **Superseded by T053**, same session: the user, reviewing this fix, asked directly why the *original* generic-named entries couldn't just show their own closest conventional designation, rather than adding a 7th duplicate-grade entry alongside unlabeled ones — a better answer to the same underlying regression. T053 replaces this approach; the 7th entry was removed again.
- [X] T053 Reverted T052's 7th entry and instead populated `material_number`/`short_notation` directly on all 6 original bundled metals in `src/mfgparams/data/materials.toml`, each with its single most conventional EN/DIN designation, fact-checked via web search rather than recalled from training data (a real reference-tool has a real accuracy stake): Mild Steel → S235JR/1.0038 (EN 10025-2, already used in T035/round 1); Stainless Steel → X5CrNi18-10/1.4301 (AISI 304, already used in T035/round 1); Brass → CuZn37/2.0321 (already used in T035/round 1); Aluminum → AlSi1MgMn/3.2315 (EN AW-6082, "one of the most widely used" wrought aluminum alloys per search); Cast Iron → EN-GJL-200/EN-JL1030 (DIN EN 1561's baseline grey-iron grade per search); Titanium → Ti99.8/3.7035 (Grade 2, "the most widely used titanium alloy in all product forms for industrial service" per search). `materials.toml`'s top-of-file comment reframes the philosophy: each value is an explicitly *representative* default (documented as such), not a claim that the generic family name and the specific grade are one and the same — accepting the same conceptual risk the review loop originally flagged, as a deliberate, human-directed tradeoff in favor of out-of-the-box usability. Reverted T052's changes to `README.md`/`quickstart.md`/test files back toward their pre-T052 shape (6 bundled metals, not 7), with `TestBundledNotations` parametrized over all 6 with real values instead of asserting `None`. Quickstart.md's Scenario 1/2 counts re-verified against the real registry merge (8 materials total, 3 "steel" matches) exactly matching the feature's original, first-draft numbers.
