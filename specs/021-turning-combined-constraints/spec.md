...
A machinist who must stay within a known available power limit (e.g., the lathe's rated power) but also needs to run a specific feed per rotation — for example, to meet a surface-finish requirement — selects a calculation mode where both the available power and the feed per rotation are fixed inputs, so the module solves for the highest spindle speed that keeps the operation within the power budget at that exact feed — never above the cutting-speed-derived reference speed standard mode uses at that feed, so a generous budget alone does not raise the recommendation further — instead of deriving feed from the material/tool's reference value the way the existing power-constrained mode does.
...
1. **Given** a user selects the power-and-feed-constrained mode and supplies both an available power and a target feed per rotation, **When** they request a calculation, **Then** the module solves for the highest spindle speed feasible within the supplied power at the supplied feed — never exceeding the material's/tool's own cutting-speed-derived reference spindle speed, even if the supplied power would allow a higher one (FR-003) — and computes machining time, cutting force, and power from that resulting spindle speed and the supplied feed.
...
- **FR-003**: The system MUST provide a power-and-feed-constrained turning calculation mode in which the caller supplies both an available power and a target feed per rotation directly, and the module solves for the highest spindle speed feasible within that available power at that feed, then computes machining time, cutting force, and power from the resulting spindle speed and the supplied feed. This spindle speed is never solved *above* the material's/tool's own cutting-speed-derived reference value (the same reference speed standard mode uses) — a surplus of available power beyond what that reference speed requires does not raise the recommended spindle speed further, mirroring the existing power-constrained mode's identical ceiling (Copilot review finding on PR #102: an earlier draft of this requirement did not make this ceiling explicit, reading as if any available power should be fully exploited).
