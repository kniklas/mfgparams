...
- [X] T020 [P] [US2] Unit tests for the new `calculate_turning_power_and_feed_constrained_metrics()` (nominal-sufficient-budget no-op case, a below-nominal-but-within-`math.isclose(rel_tol=1e-9)`-tolerance no-op boundary case -- not exact equality, which the preceding `<=` short-circuit already covers and which the boundary case must stay strictly below to actually exercise `math.isclose()` itself, corrected by a Copilot review finding on PR #102 that caught this description still naming the superseded "exact-equality" mechanism after the test was fixed to exercise the tolerance path instead -- reduced-spindle-speed case, zero budget, negative budget (added by a separate Copilot review finding on PR #102 that caught this description promising negative-budget coverage the original test file never shipped), subnormal feed underflow, arbitrary-precision-int budget) in new `tests/unit/processes/machining/turning/test_formulas_power_and_feed_constrained.py`, mirroring `test_formulas_at_rpm.py`'s existing `POWER_CONSTRAINED` test coverage
...
- [X] T044 **REQUIRED (Constitution Principle XIII)** Manually verify, against a real terminal, both new modes' field rows (Target spindle speed + Feed rate per rotation + optional Available power for `ROTATION_AND_FEED_CONSTRAINED`; Feed rate per rotation + required Available power for `POWER_AND_FEED_CONSTRAINED`) and the Machining menu's hide-on-select/restore-on-exit behavior for all three operations — layout, focus highlighting, and visibility are not directly asserted by the headless integration tests above. If the implementing agent has no access to a real terminal, leave this task unchecked and say so rather than marking it complete on the agent's behalf. Verified 2026-09-18 by the user directly, against a real terminal on this branch (`python -m mfgparams.console`): both new modes' field rows render and lay out correctly, and the Machining menu hide-on-select/restore-on-exit behavior (including selection preservation) was confirmed for all three operations.
