...
:class:`~mfgparams.models.CalculationResult` whose ``error`` is set and
...
def _to_metric(value: float, unit_system: UnitSystem) -> float:
    """Thin alias for :func:`~mfgparams.units.to_metric_length`.

    The guard this function introduced now lives in
    :mod:`mfgparams.units` so drilling's imperial path can share it
...
def calculate_milling(
    ...
    Shared by :func:`mfgparams.calculate_end_milling` and
    :func:`mfgparams.calculate_face_milling`; see
